...
st.write("")
#st.subheader("Using Neural Networks to Predict Order, Species, and Family")
...
#        return order_names[int(preds2)-1]
    ...
    def predict_birds_order_with_prob(file):
        best_model_1 = tf.keras.models.load_model('saved_models/order_est_v2.h5')
        preds1=best_model_1.predict(file)
        preds2=np.argmax(preds1,axis=1)
    ...
    def predict_birds_family_with_prob(file):
        best_model_1f = tf.keras.models.load_model('saved_models/species_est_g.h5')
    ...
    def predict_birds_species_with_prob(file):
        best_model_1f = tf.keras.models.load_model('saved_models/species_t3.h5')
        preds1f=best_model_1f.predict(file)
        preds2f=np.argmax(preds1f,axis=1)
        max_predf=np.max(preds1f)
        max_pred_percf=max_predf*100
    ...
    if label_of_probability[1]=='Anseriformes':
        ...
        st.write("Grebes are aquatic diving birds in the order Podicipediformes. Grebes are widely distributed birds of freshwater, with some species also occurring in marine habitats during migration and winter. The order contains a single family, the Podicipedidae, which includes 22 species in six extant genera.")
